...
optparse = OptionParser.new do |opts|
  ...
  opts.on "-c", "--accents NUMS", "a:b - Numbers of accents to be pointed in each half-verse" do |str|
    a1, a2 = str.split ':'
    if a1 && a1 != "" then
      setup[:output][:accents][0] = a1.to_i
    end
    if a2 && a2 != "" then
      setup[:output][:accents][1] = a2.to_i
    end
  end
  ...
  opts.on "-P", "--preparatory-syllables NUMS", "a:b - How many preparatory syllables in each half-verse" do |str|
    a1, a2 = str.split ':'
    if a1 && a1 != "" then
      setup[:output][:preparatory][0] = a1.to_i
    end
    if a2 && a2 != "" then
      setup[:output][:preparatory][1] = a2.to_i
    end
  end
  opts.on "-s", "--accents-style SYM", "underline (default) | bold" do |s|
    sym = s.to_sym
    setup[:output][:accent_style] = sym
  end
  ...
  opts.on "-S", "--split-verses", "Each verse part on it's own line" do
    setup[:output][:novydvur_newlines] = true # like in the psalter of the Novy Dvur Trappist abbey
  end
